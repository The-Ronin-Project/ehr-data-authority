...
class KafkaPublisherTest {
    ...
    private val eventPatient = EventPatient().apply {
        resourceType = "Patient"
        id = "tenant-1234"
    }

    private val practitionerRole = PractitionerRole(
        id = Id("tenant-5678")
    )
    private val eventPractitionerRole = EventPractitionerRole().apply {
    ...
    fun `supports new resources`() {
        ...
        every { kafkaClient.publishEvents(patientTopic, listOf(kafkaEvent)) } returns PushResponse(
            successful = listOf(
                kafkaEvent
            )
        )
    ...
    fun `supports multi-name resource types`() {
        ...
        every { kafkaClient.publishEvents(practitionerRoleTopic, listOf(kafkaEvent)) } returns PushResponse(
            successful = listOf(
                kafkaEvent
            )
        )
